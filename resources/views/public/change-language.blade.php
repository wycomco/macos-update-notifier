...
<head>
    ...
    <title>{{ __('public.change_language.title') }} - {{ config('app.name', 'Laravel') }}</title>
    ...
    <div class="min-h-screen flex flex-col justify-center py-12 sm:px-6 lg:px-8">
        <div class="sm:mx-auto sm:w-full sm:max-w-md">
            <h2 class="mt-6 text-center text-3xl font-extrabold text-gray-900 dark:text-white">
                {{ __('public.change_language.title') }}
            </h2>
            <p class="mt-2 text-center text-sm text-gray-600 dark:text-gray-400">
                {{ __('public.change_language.subtitle') }}
            </p>
        </div>
            ...
            <div class="bg-white dark:bg-gray-800 py-8 px-4 shadow sm:rounded-lg sm:px-10">
                <div class="mb-6">
                    <h3 class="text-lg font-medium text-gray-900 dark:text-white">
                        {{ __('public.change_language.current_info') }}
                    </h3>
                    <div class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                        <p><strong>{{ __('public.change_language.email') }}</strong> {{ $subscriber->email }}</p>
                        <p><strong>{{ __('public.change_language.current_language') }}</strong> {{ $subscriber->getLanguageDisplayNameWithFlag() }}</p>
                    </div>
                </div>
                ...
                <form method="POST" action="{{ route('public.language-change.update', $subscriber->unsubscribe_token) }}">
                    ...
                    <div class="mb-6">
                        <label for="language" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">
                            {{ __('public.change_language.select_language') }}
                        </label>
                    ...
                    <div class="flex flex-col space-y-3">
                        <button type="submit" class="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                            {{ __('public.change_language.update_button') }}
                        </button>
                        ...
                        <a href="/" class="w-full flex justify-center py-2 px-4 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            {{ __('public.change_language.cancel_button') }}
                        </a>
                    </div>
                </form>
                ...
                <div class="mt-6 text-center">
                    <p class="text-xs text-gray-500 dark:text-gray-400">
                        {{ __('public.change_language.need_help') }}
                        @if($subscriber->admin)
                            {!! __('public.change_language.contact_admin', ['email' => $subscriber->admin->email]) !!}
                        @else
                            {{ __('public.change_language.contact_support') }}
                        @endif
                    </p>
                </div>
            </div>
